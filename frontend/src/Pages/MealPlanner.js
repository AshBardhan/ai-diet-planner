...
const MealPlanner = () => {
  ...
  const sendFoodAlternative = (food, mealType, foodIndex, dailyMealIndex, mealIndex) => {
    axios.post(`${API_URL}/send-food-alt`, {food, mealType}).then(response => {
  ...
  const sendMealAlternative = (mealType, dailyMealIndex, mealIndex) => {
    axios.post(`${API_URL}/send-meal-alt`, {mealType}).then(response => {
        console.log(response.data);
        setMealList(oldMealList => {
            const newMealList = oldMealList.map(oldDailyMeal => {
                return {
  ...
  const sum = (items, prop) => items.reduce((a, b) => a + b[prop], 0);
    ...
    useEffect(() => {
        axios.get(`${API_URL}/send-weekly-meal`).then(response => {
           setMealList(Days.map((day, index) => {
            let _mealList = {
                day: `${day}`,
                id: index,
                allMealsDone: false,
            ...
            <div className="dashboard">
                ...
                <div className="daily-nutrition-wrapper">
                    <div className="chart-container">    
                        <div className="chart">
                            <div className="chart-info" style={{top: '110px', right: '45px'}}>Carbs<br/>40%</div>
                            <div className="chart-info" style={{top: '180px', left: '85px'}}>Fat<br/>35%</div>
                            <div className="chart-info" style={{top: '60px', left: '55px'}}>Protein<br/>25%</div>
                        </div>
                    </div>
                            ...
                            <div className="nutrients-list">
                            {
                                nutrientList.map((nutrientData) => (
                                    <div className="stat-pair stat-pair--vertical">
                                        <div className="stat-value" style={{color: nutrientData.color}}>{nutrientData.value}</div>
                                        <div className="stat-key" style={{color: nutrientData.color}}>{nutrientData.key}</div>
                                    </div>
                                ))
                            }
                            </div>
